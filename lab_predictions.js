...
function formatUpdatedDate(isoString) {
  ...
  try {
    ...
    return `Updated • ${d.toLocaleDateString("en-GB")} • ${d.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
  } catch {
    return "Updated • --";
  }
}
...
function createPickCard(pick, bucketKey) {
  ...
  return `
    <article class="prediction-card prediction-card--lab prediction-card--compact">
      ...
      <h3 class="prediction-card__match">${escapeHtml(pick.match || "-")}</h3>
      <p class="prediction-card__league">🏆 ${escapeHtml(pick.league || "-")}</p>
      ...
      <details class="prediction-card__details">
        <summary>Detailed AI analysis</summary>
...
async function loadLabPredictions() {
  try {
    const response = await fetch(`./lab_predictions.json?v=${Date.now()}`, { cache: "no-store" });
    ...
    if (updatedEl) updatedEl.textContent = "Updated • error";
    if (container) {
      container.innerHTML = `
        <div class="prediction-card prediction-card--lab">
